...
sell_data = pd.read_csv(adress + 'rawdata/sale_data.csv', dtype='str')
...
#sell_data['城市'] = sell_data['土地位置建物門牌'].str[:3]
#sell_data.loc[:,'城市'] = sell_data['城市'].apply(lambda x : str(x).replace('台','臺')) # 文字統一
#sell_data.loc[:,'城市'] = sell_data['城市'].apply(lambda x : str(x).replace('巿','市')) # 文字統一

#sell_data['土地位置建物門牌'].str[:3]
...
sell_data = sell_data[(sell_data['建築完成年月'].str[3:7].astype('float')!=229) & (sell_data['建築完成年月'].str[3:7].astype('float')!=230) & (sell_data['建築完成年月'].str[3:7].astype('float')!=231) & (sell_data['建築完成年月'].str[3:7].astype('float')!=431) & 
...
# '建物現況格局-衛' 經人員檢測249間以上為極端值
sell_data = sell_data[sell_data['建物現況格局-衛'].astype("float")<250]
...
data = data[~data.Place_id.isna()] # 剔除mapping不到的資料

# 交易標的
data['Type'] = sell_data['交易標的'].copy()
# {'車位': 0, '房地(土地+建物)': 1, '房地(土地+建物)+車位': 2, '建物': 3}
...
# output data
data.to_csv(adress + 'output_feature/sale_data_feature_martin.csv')
