...
file_path = os.path.join(dir_path, 'sale_data.csv')
raw_df = pd.read_csv(file_path)

# 將指定欄位篩選出來
clean_df = raw_df.copy(deep=True)
clean_df = clean_df[ ['編號', '都市土地使用分區', '非都市土地使用編定', '交易筆棟數', '備註'] ]

################################
######### Delete data ##########
################################
# delete data if '交易筆棟數'=='土地0建物0車位0', 共丟棄50個row
clean_df = clean_df[ clean_df['交易筆棟數'] != '土地0建物0車位0' ]
...
clean_df.loc[mis_df.index, '非都市土地使用編定'] = clean_df.loc[mis_df.index,'都市土地使用分區'].apply(lambda x: x.split('：')[-1])
...
non_city_label = {'農牧用地':1, 
                 ...
                 '墳墓用地':20, 
                 '鹽業用地':21}
clean_df['非都市土地使用編定'].fillna(0, inplace=True)
clean_df['非都市土地使用編定'] = clean_df['非都市土地使用編定'].replace(non_city_label)
...
clean_df['Transaction_Land'] = clean_df['交易筆棟數'].apply(lambda x: int(re.findall(r'.{2}[0-9]{1}', x)[0][-1]))
clean_df['Transaction_Building'] = clean_df['交易筆棟數'].apply(lambda x: int(re.findall(r'.{2}[0-9]{1}', x)[1][-1]))
clean_df['Transaction_Parking'] = clean_df['交易筆棟數'].apply(lambda x: int(re.findall(r'.{2}[0-9]{1}', x)[2][-1]))

##### 備註 #####
# 將'備註'欄位ˋ中的空值進行轉換，再查找關鍵字後建立對應的類別欄位，空值有考慮(其餘類別欄位為0不一定代表無字串)
clean_df['備註'].fillna('無備註', inplace=True)
note_dic = {
    'Note_Null':['無備註'],
    ...
    'Note_Decoration':['含裝潢', '附裝潢', '含裝修'], # 裝潢
    'Note_Furniture':['含傢俱'], # 傢俱
    'Note_Layer':['夾層'], # 夾層
...
for key in note_dic:
    clean_df[key] = clean_df['備註'].apply(lambda x: note_features(key, x))
...
clean_df.drop(columns=['都市土地使用分區', '備註', '交易筆棟數'], inplace=True)
columns_map_sam = {
    '非都市土地使用編定':'Non_City_Land_Code',
}
clean_df.rename(columns=columns_map_sam, inplace=True)
# export the dataframe into csv file
output_path = os.path.join(dir_path, 'sale_data_feature_sam.csv')
